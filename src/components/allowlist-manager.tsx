'use client';

import { useState } from 'react';

export function AllowlistManager({ items }: { items: string[] }) {
  const [newItem, setNewItem] = useState('');

  const addItem = async () => {
    if (!newItem.trim()) return;
    await fetch('/api/admin/allowlist', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ value: newItem.trim(), action: 'add' }),
    });
    setNewItem('');
    window.location.reload();
  };

  const removeItem = async (value: string) => {
    await fetch('/api/admin/allowlist', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ value, action: 'remove' }),
    });
    window.location.reload();
  };

  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <input
          type="text"
          value={newItem}
          onChange={(e) => setNewItem(e.target.value)}
          placeholder="Add email or domain..."
          className="flex-1 h-9 px-3 rounded border text-sm bg-background focus:outline-none focus:ring-1 focus:ring-ring"
        />
        <button onClick={addItem} className="h-9 px-4 rounded bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity">
          Add
        </button>
      </div>
      <div className="border rounded-lg divide-y max-h-96 overflow-y-auto">
        {items.map((item) => (
          <div key={item} className="flex items-center justify-between p-3 text-sm">
            <span className="font-mono">{item}</span>
            <button onClick={() => removeItem(item)} className="text-xs text-destructive hover:underline">Remove</button>
          </div>
        ))}
      </div>
    </div>
  );
}
